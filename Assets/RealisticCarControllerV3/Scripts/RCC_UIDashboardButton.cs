...
public class RCC_UIDashboardButton : MonoBehaviour, IPointerClickHandler {
	...
	void Start(){
		...
		if(_buttonType == ButtonType.Gear && GameObject.FindGameObjectWithTag("GearSlider").GetComponent<Scrollbar>())
		{

			//gearSlider.onValueChanged.AddListener (delegate {ChangeGear ();});
			gearSlider = GameObject.FindGameObjectWithTag("GearSlider").GetComponent<Scrollbar>();
    ...
    public void ChangeGear(){
		...
		if(gearDirection == InputManager.Instace.OnGear())
			return;

		//gearDirection = Mathf.CeilToInt(gearSlider.value * 2);

		gearDirection = InputManager.Instace.OnGear();

//		RCC_SceneManager.Instance.activePlayerVehicle.semiAutomaticGear = true;

		Debug.Log("Gear Direction "+gearDirection);

		gearSlider.value = gearDirection;
